...
function Export-Leases { param([string]$Server,[string]$ScopeId,[string]$Path)
    $x=@(Get-DhcpServerv4Lease -ComputerName $Server -ScopeId $ScopeId -AllLeases -ErrorAction Stop | Select-Object @{n='Server';e={$Server}},ScopeId,IPAddress,ClientId,HostName,AddressState,LeaseExpiryTime,Description)
