...
# Revision 1.5: Add per-scope Phase I recovery exports while retaining full server backups in both phases.
# Revision 1.4: Correct nested-array lease exports and wait for actual lease replication.
...
function Get-DhcpLeasesSafe { param([string]$Server,[string]$ScopeId)
    ...
    # Emit individual lease objects. The caller wraps the command in @(), which
    # preserves a true zero/one/many collection without creating a nested array.
    Write-Output $leases
}
function Export-Leases { param([string]$Server,[string]$ScopeId,[string]$Path)
    ...
    if($invalid.Count){throw "DHCP lease query for scope $ScopeId on $Server returned $($invalid.Count) object(s) without IPAddress or ClientId. Export and validation stopped to prevent false-positive lease counts."}
    $x | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $Path
    Write-Output -NoEnumerate $x
}
function Wait-LeaseReplication { param([string]$Server,[string]$ScopeId,[array]$Baseline,[string]$Path,[int]$TimeoutSeconds=180)
    $end=(Get-Date).AddSeconds($TimeoutSeconds)
    do {
        $actual=@(Export-Leases -Server $Server -ScopeId $ScopeId -Path $Path)
        $actualKeys=@{}; foreach($lease in $actual){$actualKeys[(Lease-Key $lease)]=$true}
        $missing=@($Baseline | Where-Object {-not $actualKeys.ContainsKey((Lease-Key $_))})
        if($missing.Count -eq 0){Write-Output -NoEnumerate $actual; return}
        Write-Log "Waiting for lease replication to $Server for scope $ScopeId. Actual=$($actual.Count); missing=$($missing.Count)." 'WARN'
        Start-Sleep -Seconds 5
        try{Invoke-DhcpServerv4FailoverReplication -ComputerName $Server -ScopeId $ScopeId -Force -ErrorAction Stop | Out-Null}catch{Write-Log $_.Exception.Message 'WARN'}
    }while((Get-Date)-lt $end)
    throw "Lease replication to $Server for scope $ScopeId did not complete within $TimeoutSeconds seconds. Missing=$($missing.Count) of baseline=$($Baseline.Count)."
}
...
function Export-ScopeRecoveryPackage { param([string]$Root,[string]$Server,[string]$ScopeId,[string]$ScopeName)
    ...
    if(-not (Test-Path $xml -PathType Leaf) -or (Get-Item $xml).Length -le 0){throw "Per-scope DHCP XML export was not created for scope $ScopeId."}
    ...
    $reimported=@(Import-Csv $leaseCsv)
    if($reimported.Count -ne $leases.Count){throw "Per-scope lease CSV validation failed for $ScopeId. Exported=$($leases.Count); reimported=$($reimported.Count)."}
    ...
    $manifest=[pscustomobject]@{
        Created=Get-Date; SourceServer=$Server; ScopeId=$ScopeId; ScopeName=$ScopeName
        LeaseCount=$leases.Count; ReservationCount=$reservations.Count
        XmlFile=(Split-Path $xml -Leaf); LeaseCsv=(Split-Path $leaseCsv -Leaf); ReservationCsv=(Split-Path $reservationCsv -Leaf)
        XmlSHA256=(Get-FileHash $xml -Algorithm SHA256).Hash
        LeaseCsvSHA256=(Get-FileHash $leaseCsv -Algorithm SHA256).Hash
        ReservationCsvSHA256=(Get-FileHash $reservationCsv -Algorithm SHA256).Hash
    }
    ...
    return [pscustomobject]@{Path=$package;LeaseCount=$leases.Count;ReservationCount=$reservations.Count}
}
    ...
    try {
        if($phase -eq 1){
            ...
            if($PreflightOnly){
                ...
                $scopePackage=Export-ScopeRecoveryPackage -Root $r.BackupRoot -Server $env:COMPUTERNAME -ScopeId $scope -ScopeName $scopeName
                ...
                Start-Step 11 13 'Validate leases retained on legacy Edge'
                ...
                $eqb=Wait-LeaseReplication -Server $r.NewHub -ScopeId $scope -Baseline $combined -Path (Join-Path $dir 'EQB-Leases-AfterSync.csv')
            ...
            if($PreflightOnly){
                ...
                $new=Wait-LeaseReplication -Server $r.NewEdge -ScopeId $scope -Baseline $base -Path (Join-Path $dir 'NewEdge-Leases-Final.csv')
                ...
                if($unknown.Count){Write-Log "$($unknown.Count) replicated lease(s) have unknown/empty AddressState because this DHCP module build did not expose lease state. IPAddress and ClientId replication were validated." 'WARN'}
