...
# Revision 2.4: Phase I is anchored to the retained legacy Edge, verifies LegacyEdge-to-LegacyHub management access, and proves the scope is removed from LegacyHub after detach.
...
function Get-DhcpScopeFromInventory { param([string]$Server,[string]$ScopeId)
    # Query the full inventory so an absent scope is distinguishable from transport failure.
    $inventory=@(Get-DhcpServerv4Scope -ComputerName $Server -ErrorAction Stop)
    return @($inventory | Where-Object { ([string]$_.ScopeId).Trim() -eq $ScopeId })
}
...
function Test-SharedRelationshipPlans { param([array]$Rows)
    foreach($plan in @(
        [pscustomobject]@{Phase=1;NameColumn='TempRelationship';ActionColumn='TempRelationshipAction';Left='LegacyEdge';Right='NewHub'},
        [pscustomobject]@{Phase=2;NameColumn='FinalRelationship';ActionColumn='RelationshipAction';Left='NewHub';Right='NewEdge'}
    )){
    ...
    try {
        if($phase -eq 1){
            ...
            if($fo){
                ...
                if(($partnerShort -ieq $legacyHubShort) -and ([string]$fo.Mode -in @('LoadBalance','HotStandby'))){
                    $phase1State='OriginalLegacyFailover'
                    Write-Log "Accepted legacy source relationship. Name=$($fo.Name); Mode=$($fo.Mode); Partner=$($fo.PartnerServer)." 'PASS'
                ...
                } else {throw "Scope $scope is in an unexpected relationship. Name=$($fo.Name); Mode=$($fo.Mode); Partner=$($fo.PartnerServer)."}
            }
            ...
            if($phase1State -eq 'OriginalLegacyFailover'){
                Write-Log "Validating required management direction from retained LegacyEdge $env:COMPUTERNAME to LegacyHub $($r.LegacyHub). Reverse-direction reachability is not required by this workflow." 'INFO'
                $legacyHubScopeBefore=@(Get-DhcpScopeFromInventory -Server $r.LegacyHub -ScopeId $scope)
                if($legacyHubScopeBefore.Count -ne 1){throw "LegacyHub inventory validation expected exactly one scope $scope on $($r.LegacyHub); found $($legacyHubScopeBefore.Count). No DHCP changes were made."}
                Write-Log "LegacyEdge-to-LegacyHub management validation passed for scope $scope." 'PASS'
            }
            ...
            if($PreflightOnly){
                ...
                Start-Step 10 13 'Detach scope from legacy Hub, retain legacy Edge'
                if($phase1State -eq 'OriginalLegacyFailover'){
                ...
                Start-Step 11 13 'Validate scope removed from legacy Hub and retained on legacy Edge'
                $localScopeAfter=Get-DhcpServerv4Scope -ScopeId $scope -ErrorAction Stop
                if(-not $localScopeAfter){throw "Scope $scope is not present on retained LegacyEdge $env:COMPUTERNAME after detach."}
                $legacyHubScopeAfter=@(Get-DhcpScopeFromInventory -Server $r.LegacyHub -ScopeId $scope)
                if($legacyHubScopeAfter.Count -ne 0){
                    throw "Safety stop: scope $scope is still present on LegacyHub $($r.LegacyHub) after legacy failover removal. The script will not create the temporary relationship."
                }
                Write-Log "Post-detach placement verified: scope $scope is retained on LegacyEdge $env:COMPUTERNAME and absent from LegacyHub $($r.LegacyHub)." 'PASS'
                ...
                Start-Step 12 13 'Create or join temporary EQB Hot Standby relationship'
                ...
                Start-Step 13 13 'Validate EQB relationship and lease replication'
            ...
            if($phase2State -eq 'TemporaryLegacyEdgeFailover'){
                ...
                Wait-FailoverNormal $env:COMPUTERNAME $scopeFo.Name | Out-Null
                Assert-FailoverServerRole -Server $env:COMPUTERNAME -Name $scopeFo.Name -ExpectedRole 'Standby' | Out-Null
                Assert-FailoverServerRole -Server $r.NewEdge -Name $scopeFo.Name -ExpectedRole 'Active' | Out-Null
                Complete-Step "Final relationship already established=$($scopeFo.Name); state=Normal; Hub=Standby; Edge=Active"
            } else {
            ...
            if($PreflightOnly){
                ...
                if($currentFo){
                    ...
                    if($nameCollision){
                        ...
                        Complete-Step "Created final relationship=$($r.FinalRelationship); Standby=$env:COMPUTERNAME; Active=$($r.NewEdge)"
                    }
                }
                $script:LastCompletedStep=$script:CurrentStep
                ...
                Assert-FailoverServerRole -Server $env:COMPUTERNAME -Name $r.FinalRelationship -ExpectedRole 'Standby' | Out-Null
                Assert-FailoverServerRole -Server $r.NewEdge -Name $r.FinalRelationship -ExpectedRole 'Active' | Out-Null
                ...
                Complete-Step "NewEdge=$($new.Count); baseline=$($base.Count); missing=0; relationship=$($scopeFoFinal.Name); mode=$($scopeFoFinal.Mode); Hub=Standby; Edge=Active"
                $script:LastCompletedStep=$script:CurrentStep
                $result='PASS'
