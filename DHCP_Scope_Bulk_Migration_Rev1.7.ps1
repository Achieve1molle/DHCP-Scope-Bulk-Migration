...
foreach($r in $rows){
    ...
    [void]$summary.Add([pscustomobject]@{Time=Get-Date;Phase=$phase;ScopeId=$scope;ScopeName=$scopeName;LegacyEdge=$r.LegacyEdge;NewHub=$r.NewHub;NewEdge=$r.NewEdge;ReservePercent=$rp;AllowZeroLeases=$allowZeroLeases;Result=$result;Message=$message})
