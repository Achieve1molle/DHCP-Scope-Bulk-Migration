...
# Revision 2.0: Shared Temp/Final relationship reuse; flexible Phase I source states; NewHub active.
    ...
    foreach($plan in @(
        ...
        foreach($group in @($planRows | Group-Object {([string]$_.$($plan.NameColumn)).Trim().ToLowerInvariant()})){
            $pairs=@($group.Group | ForEach-Object {
                '{0}|{1}' -f (([string]$_.$($plan.Left)).Trim().ToLowerInvariant()),(([string]$_.$($plan.Right)).Trim().ToLowerInvariant())
...
function Test-Phase2RelationshipPlan { param([array]$Rows)
    ...
    Write-Log "Phase II relationship-name validation passed. Shared FinalRelationship values are allowed for a common NewHub/NewEdge pair." 'PASS'
        ...
        if($phase -eq 1){
            ...
            if($fo){
                ...
                if(($partnerShort -ieq $legacyHubShort) -and ([string]$fo.Mode -in @('LoadBalance','HotStandby'))){
                    ...
                    throw "Scope $scope is in an unexpected relationship. Name=$($fo.Name); Mode=$($fo.Mode); Partner=$($fo.PartnerServer). Expected no relationship, LoadBalance/HotStandby with LegacyHub '$($r.LegacyHub)', or temporary HotStandby '$($r.TempRelationship)' with NewHub '$($r.NewHub)'."
                }
            }
            if($phase1State -eq 'Detached'){Write-Log "Scope $scope has no failover relationship. Continuing from LegacyEdge as an already-detached state." 'WARN'}
            ...
            if($PreflightOnly){
                ...
                if($scopeFo){
                    ...
                    if($tempRelationshipAction -eq 'Create' -and $tempFo){throw "TempRelationshipAction=Create requires a new name, but '$($r.TempRelationship)' already exists."}
                    if($tempFo){
            ...
            if($PreflightOnly){
                ...
                if($currentFo){
                    ...
                    if($relationshipAction -eq 'Create' -and $nameCollision){throw "RelationshipAction=Create requires a new name, but '$($r.FinalRelationship)' already exists."}
                    if($nameCollision){
                        ...
                        if($PSCmdlet.ShouldProcess("Scope $scope","Create final Hot Standby with new Edge $($r.NewEdge)")){
                            Add-DhcpServerv4Failover -Name $r.FinalRelationship -PartnerServer $r.NewEdge -ScopeId $scope -ServerRole Active -ReservePercent $rp -MaxClientLeadTime ([timespan]$mclt) -Force -ErrorAction Stop | Out-Null
                        }
                        Complete-Step "Created final relationship=$($r.FinalRelationship); Active=$env:COMPUTERNAME; Standby=$($r.NewEdge)"
                    }
                }
                $script:LastCompletedStep=$script:CurrentStep
