...
function Get-DhcpLeasesSafe { param([string]$Server,[string]$ScopeId)
    ...
    Set-StrictMode -Off
    return @(Get-DhcpServerv4Lease -ComputerName $Server -ScopeId $ScopeId -AllLeases -ErrorAction Stop)
}
function Export-Leases { param([string]$Server,[string]$ScopeId,[string]$Path)
    ...
    $x=@($raw | ForEach-Object {
        ...
        [pscustomobject]@{
            ...
            AddressState    = if($_.PSObject.Properties['AddressState']){[string]$_.AddressState}elseif($_.PSObject.Properties['State']){[string]$_.State}else{''}
            LeaseExpiryTime = if($_.PSObject.Properties['LeaseExpiryTime']){$_.LeaseExpiryTime}else{$null}
...
function Export-Reservations { param([string]$Server,[string]$ScopeId,[string]$Path)
    ...
    $x | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $Path; return $x
}
